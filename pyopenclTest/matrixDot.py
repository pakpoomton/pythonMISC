...
if __name__ == "__main__":
    ...
    N = 3
    #ok somehow we need this as type thing
    vector = numpy.random.random((N,1)).astype(numpy.float32)
    matrix = numpy.random.random((N,N)).astype(numpy.float32)

    #vector = numpy.ones((4,1)).astype(numpy.float32)
    #matrix = numpy.ones((4,4)).astype(numpy.float32)
    ...
    matrix_dot_vector = numpy.zeros(N, numpy.float32)
    ...
    ## Step 11. Move the kernel output data to host memory.
    ...
    # now compare to regular numpy matrix dot product
    start = default_timer()
    reg_matrix_dot_vector = numpy.dot(matrix, vector)
    print(reg_matrix_dot_vector)
    print default_timer() - start
