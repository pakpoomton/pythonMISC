...
depth = 60. # in this case the width of space to generate  pattern
...
D = .001*sc*sc # diffusion rate of s1 and s2 signal
Ka = 0.02 # maximal switching rate of c0->c1s and c0->c2s
Km = 100 # [s2] and [s1] at half maximal swiching rate of c0->c1s and c0->c2s
A = 10000 # production rate of s1 and s2 from from c1 and c2
Ga = .01 #maximal degradation rate s1 and s2
...
Nz = 800. # number of discrete step in space
...
#for i in range(1,  50):
    ...
    inC1s = C0[:,i-1]*S2[:,i-1]*Ka/(S2[:,i-1]+Km2)
    C1s[:,0] = np.copy(inC1s)
    ...
    S2[1:-1,i] = S2[1:-1,i] + .1*A*C2[1:-1,i] - 10*Ga*S2[1:-1,i]
...
xlabel = map(str, np.around(xbound*depth/Nz/sc, decimals = 1))
ybound = [0, Nt]
